...
class Glyph(object):
    ...
    @:param delta: The amount to move after pasting the glyph.
        Both values must be positive or zero. Glyphs do not alpha composite/mask.
    @:param src_bbox: The box to take from the atlas to get the glyph's bitmap. x1/y1/x2/y2.
    @:param dst_bbox: The box to paste the glyph's bitmap to on an image, as offsets from the glyph's position. x1/y1/x2/y2.
    """

    character: bytes = b'\x00'
    delta: Tuple[int, int] = (0, 0)
    src_bbox: Tuple[int, int, int, int] = (0, 0, 0, 0)
    dst_bbox: Tuple[int, int, int, int] = (0, 0, 0, 0)
...
class PILFont:
    ...
    @:param atlas: The glyph atlas.
        Must be of mode 1 or L, where L is the alpha channel.
